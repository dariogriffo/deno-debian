...
if [ -z "$DENO_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <deno_version> <build_version>"
    echo "Example: $0 2.7.2 1"
    exit 1
fi

PACKAGE_NAME="deno"
ORIG_TARBALL="${PACKAGE_NAME}_${DENO_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${DENO_VERSION}"

echo "Creating Debian/Ubuntu source packages for deno ${DENO_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/denoland/deno/archive/refs/tags/v${DENO_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${DENO_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as deno-x.y.z/ which matches our BUILD_DIR
...
deno (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${DENO_VERSION}.
